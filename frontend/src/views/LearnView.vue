<template>
  <div class="wfl-page learn-page" v-loading="loading">
    ...
    <div v-if="!plan || plan.total === 0" class="wfl-card empty-card">
      <el-icon :size="54" color="#c7d2fe">
        <Reading />
      </el-icon>
      <h2>今天还没有学习计划</h2>
      <p>
        当前词书：<strong>{{ plan?.bookName || '四级核心词汇' }}</strong
        >，你的每日目标是 <strong>{{ userStore.user?.dailyWordGoal || 20 }}</strong> 个新词。
    ...
    <div v-else-if="showContinue" class="wfl-card empty-card">
      <el-icon :size="54" color="#6366f1">
        <Refresh />
      </el-icon>
      <h2>继续学习</h2>
      <p>
        今日计划来自「{{ plan.bookName }}」，已完成
        <strong>{{ plan.completed }} / {{ plan.total }}</strong> 个单词，还剩
        <strong>{{ Math.max(0, plan.total - plan.completed) }}</strong> 个待完成。
      </p>
      <p class="continue-note">
        每日目标
        {{ plan.dailyWordGoal }} 个。点击继续后，系统会先检测你是否更换了词书或调整了每日目标，
        并自动同步最新计划。
      </p>
      <div class="empty-actions">
    ...
    >
      <div class="pass-body">
        <div class="pass-head">
          ...
          <div
            v-if="passResult?.score !== null && passResult?.score !== undefined"
            class="pass-score"
          >
        ...
        <div v-if="passResult?.errors && passResult.errors.length" class="pass-block">
          ...
          <div v-for="(error, index) in passResult.errors" :key="index" class="pass-error">
            <div class="pass-error-row">
              <span class="pass-error-label">期望表达</span>{{ error.expected }}
            </div>
            <div class="pass-error-row">
              <span class="pass-error-label">你的表达</span>{{ error.user }}
            </div>
            <div class="pass-error-row">
              <span class="pass-error-label">修改建议</span>{{ error.suggestion }}
            </div>
          </div>
        </div>
        ...
        <div v-if="passPair" class="pass-block pair-block">
          ...
          <div class="pair-grid">
            ...
            <div class="pair-col">
              <div class="pair-label">
                {{ passPair.standardLabel }}
              </div>
              <div class="pair-text pair-standard">
                {{ passPair.standard }}
              </div>
            </div>
          </div>
      ...
      <template #footer>
        <el-button type="success" size="large" class="pass-confirm" @click="confirmPass">
          继续
        </el-button>
      </template>
    </el-dialog>
...
async function handleEn(option: string) {
  ...
  busy.value = true;
  try {
    const checked = await learningApi.checkEn(current.value.id, option);
    if (checked.correct) {
      if (!practice.value) {
        practice.value = await learningApi.getPracticeSentence(current.value.id);
      }
